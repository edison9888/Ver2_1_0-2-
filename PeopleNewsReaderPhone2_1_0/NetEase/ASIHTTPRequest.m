...
- (NSString *)responseString
{
	...
	//NSLog(@"data:%@",data);
    //NSString *strTemp = [[NSString alloc] initWithData:data encoding:CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingGB_18030_2000)];
	...
	// If zero bytes were read, wait for the EOF to come.
		...
		//Otherwise, let's add the data to our in-memory store
			...
			if ([self isResponseCompressed] && ![self shouldWaitToInflateCompressedResponses]) {
				...
				fileName = [fileName stringByAppendingPathComponent:@"people.xml"];
				if (![rawResponseData writeToFile:fileName atomically:NO]) {
					NSLog(@"FileName1111:%@", fileName);
				}
